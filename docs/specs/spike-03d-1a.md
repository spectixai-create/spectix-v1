...
| IMPORTANT 4: storage egress trigger ($5/month) is dead — no monitoring exists                             | TECH_DEBT 11k owner/trigger revised to CEO monthly invoice review; trigger is first Supabase invoice line item showing storage egress > 0.                                            |
| IMPORTANT 5: U-NEW-3 sentinel masks Claude reachability                                                   | Failure path actor_id uses `DEFAULT_MODEL` from client.ts (single source of truth) for subtype-stage llm_call failures. No wrapper sentinel is introduced.                            |
...
if (subtypeResult && !subtypeResult.skipped) {
  await step.run('upsert-pass-subtype-cost', async () => {
    ...
    if (error) {
      throw new Error(
        `upsert_pass_increment (subtype) failed: ${error.message}`,
      );
    }
  });
}
...
const finalizeOutcome = (await step.run('finalize-processed', async () => {
  ...
  if (broadAuditError) {
    logger.error('[audit-failure-broad]', {
      documentId,
      error: broadAuditError,
    });
  }
  ...
  if (
    ...
    subtypeResult.llmReturnedRaw !== null
  ) {
    subtypeAuditDetails.llm_returned_invalid_subtype =
  ...
  if (subtypeAuditError) {
    logger.error('[audit-failure-subtype]', {
      documentId,
      error: subtypeAuditError,
    });
  }
...
- **11k**: subtype classifier downloads the same storage object as broad classifier. Future optimization: pass bytes between steps via Inngest step output. **Owner:** CEO (monthly review of Supabase egress in dashboard). **Trigger:** first Supabase invoice line item showing storage egress > 0.
...
- **11n**: watchdog threshold (currently 5 minutes per HR-001) was set when only broad classification ran. Two-tier classification doubles the latency budget. **Owner:** PM (weekly review during Section 7 maintenance pass of PM_REVIEW_CHECKLIST). **Trigger:** when 95th percentile of audit_log.details.processing_time_ms exceeds 60s over a 7-day window.
...
- Removed any subtype-specific force-failure env var path. U-NEW-3 unit-test evidence is the sole accepted proof for broad-cost preservation on subtype failure.
- Defined `getFailureAuditActor` explicitly with `failurePhase` and `DEFAULT_MODEL` import.
- Removed the dead subtype wrapper sentinel export; all reachable subtype wrapper failures throw as LLM failures.
...
- Added TECH_DEBT 11o for an RPC idempotency key and clarified owners/triggers for 11k and 11n.
- Corrected concurrency math: two-tier classification remains max 5 active LLM calls per claim because broad and subtype calls are sequential within each function.
- Migration 0005 now drops any prior subtype CHECK before adding the canonical 37-value constraint.
