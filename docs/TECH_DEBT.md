...
- [ ] 11r. Pass row remains `in_progress` after document-level broad extraction smoke completes. Observed in TASK-SPECTIX-001: pass 1 status stayed `in_progress` after all 8 smoke documents reached `processed`, with `llm_calls_made = 19` and `cost_usd = 0.16041`. Decision needed: whether `passes.status` belongs to claim-level pass lifecycle or should be completed when document-level processing finishes. **Severity:** medium. **Status:** not blocking PR #18 validation.
