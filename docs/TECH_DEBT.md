...
- [ ] 11q. If an extractor returns successfully but `extracted_data.kind` does not match `documents.document_type`, the row is inconsistent. Add runtime check post-extraction. **Owner:** PM weekly Section 7 query. **Trigger:** first occurrence from `select id from documents where (extracted_data->>'kind') is not null and (extracted_data->>'kind') != document_type;`.
