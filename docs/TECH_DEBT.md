...
- [x] 11r. Pass row remains `in_progress` after document-level broad extraction smoke completes. Resolved by Sprint #001: pass 1 is the claim-level document-processing pass and now completes when every document for the claim is terminal, or fails when any terminal document failed. TASK-SPECTIX-001 observed pass 1 `in_progress` after all 8 smoke documents reached `processed`, with `llm_calls_made = 19` and `cost_usd = 0.16041`. **Severity:** medium. **Status:** not blocking PR #18 validation.
